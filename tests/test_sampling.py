...
testing_params = [
    ...
    ("Inactionable", "Actionable"),
    ("Inactionable", "Monotonic"),
    ("Actionable", "Monotonic"),
    ("Monotonic", "Monotonoic"),
    ("Monotonic", "Actionable"),
    ("Monotonic", "Inactionable"),
]
...
def test_bool_data(setup_bool_data, solver):
    ...
    for sample in samples:
        if not x1_actionable:
            assert sample[0] == x[0]
        if not x2_actionable:
            assert sample[1] == x[1]
...
def test_sampling_accuracy(solver):
    ...
    count = 0
    for sample in samples:
        if (lowerx <= sample[0] <= upperx) and (lowery <= sample[1] <= uppery):
            count += 1
